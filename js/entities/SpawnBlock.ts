...
import { CRYSTAL_ENEMY_MAPPING } from './EnemyTypes';
import type { Camera, CrystalType, SpawnInfo } from '../types';
...
export class SpawnBlock {
    ...
    constructor(x: number, y: number, crystalType: CrystalType) {
        this.x = x;
        this.y = y;
     ...
     */
    addTowerXp(amount: number): boolean {
        this.towerXp += amount;
    ...
    renderLevel(ctx: CanvasRenderingContext2D, screen: { x: number; y: number }, scale: number): void {
    ...
    renderHealthBar(ctx: CanvasRenderingContext2D, screen: { x: number; y: number }, scale: number): void {
