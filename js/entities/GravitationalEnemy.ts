...
import { ENEMY_TYPES, getEnemyConfig } from './EnemyTypes';
...
interface GravitationalContext {
    otherGravEnemies?: GravitationalEnemy[];
}
...
export class GravitationalEnemy extends BaseEnemy {
    ...
    constructor(x: number, y: number, scaling: any = null) {
        const config = getEnemyConfig('gravitational', scaling);
        super(x, y, config);
        
        this.gravityRange = (config as any).gravityRange;
        this.gravityStrength = (config as any).gravityStrength;
    ...
    update(dt: number, playerX: number, playerY: number, context: GravitationalContext = {}): FireTrailInfo | null {
